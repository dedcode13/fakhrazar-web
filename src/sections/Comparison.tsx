function CheckIcon({ good }: { good: boolean }) {
  return (
    <svg viewBox="0 0 20 20" className={`w-5 h-5 shrink-0 ${good ? "text-orange-500" : "text-neutral-600"}`} fill="currentColor">
      {good ? (
  ...
  return (
    <section className="py-32 bg-black border-t border-white/5">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-4xl sm:text-5xl font-black mb-16 max-w-2xl text-white">
          خیلی‌ها قول می‌دهند، ما تحویل می‌دهیم
        </h2>

        <div className="grid md:grid-cols-2 gap-6">
          <div className="rounded-none border border-white/10 p-10">
            <h3 className="text-sm font-bold text-neutral-500 mb-8 tracking-widest">
              روش مرسوم بازار
            </h3>
            <ul className="space-y-5">
              {oldWay.map((item) => (
                <li key={item} className="flex items-center gap-3 text-neutral-400">
          ...
          <div className="rounded-none bg-neutral-900 text-white p-10">
            <h3 className="text-sm font-bold text-orange-500 mb-8 tracking-widest">
